...
def randomly_gen_uspace_url() -> list:
    ...
    for i in range(12):
        uid = random.randint(10, 100000)
        url = "https://www.sakuraaaa.com/space-uid-{}.html".format(str(uid))
        url_list.append(url)
    return url_list
...
def check_anti_cc() -> dict:
    ...
    home_page = "https://www.sakuraaaa.com/forum.php"
...
def login(username: str, password: str) -> req_Session:
    headers = {
        "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36",
        "origin": "https://www.sakuraaaa.com",
        "referer": "https://www.sakuraaaa.com/forum.php",
    }
    login_url = "https://www.sakuraaaa.com/member.php?mod=logging&action=login&loginsubmit=yes&infloat=yes&lssubmit=yes&inajax=1"
    login_data = {
...
def check_login_status(s: req_Session, number_c: int) -> bool:
    test_url = "https://www.sakuraaaa.com/home.php?mod=spacecp"
    res = s.get(test_url)
    res.raise_for_status()
    res.encoding = "utf-8"
    ...
    if len(test_title) != 0:  # 确保正则匹配到了内容，防止出现数组索引越界的情况
        if test_title[0] != "个人资料   樱花论坛 -  Powered by Discuz!":
...
def print_current_points(s: req_Session):
    test_url = "https://www.sakuraaaa.com/forum.php"
    res = s.get(test_url)
    res.raise_for_status()
    res.encoding = "utf-8"
...
if __name__ == "__main__":
    #username = ""
    #password = ""
    username = os.environ["sakuraaaa_USERNAME"]
    password = os.environ["sakuraaaa_PASSWORD"]
    #账户和密码
